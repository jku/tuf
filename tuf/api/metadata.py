...
class Metadata:
    ...
    Attributes:
        ...
        signatures: A list of Securesystemslib Signature objects, each signing
            the canonical serialized representation of 'signed'. this is a
            read-only representation of signatures_by_keyid.

        signatures_by_keyid: A dict of keyids to Securesystemslib Signature
            objects, each signing the canonical serialized representation of
            'signed'.
    """
    ...
    def __init__(self, signed: "Signed", signatures: Dict[str, Signature]):
        ...
        Args:
            signed: An instance of Targets, Snapshot, Timestamp or Root
            signatures: A dictionary of keyids to Signatures
        """
        self.signed = signed
        self.signatures_by_keyid = signatures

    @property
    def signatures(self) -> List[Signature]:
        return sorted(self.signatures_by_keyid.values())
    ...
    def sign(
        ...
        if append:
            self.signatures_by_keyid[signature.keyid] = signature
        else:
            self.signatures_by_keyid = {signature.keyid: signature}
    ...
    def verify_delegate(
        ...
        """Verifies that 'delegate' is signed with the correct threshold of
        ...
        Raises:
            UnsignedMetadataError: 'delegate' was not signed with the threshold
                of keys for 'role_name'
        """

        # Find the keys and role in our metadata
        if self.signed.type == "root":
            keys: Dict[str, Key] = self.signed.keys
            role: Optional[Role] = self.signed.roles.get(role_name, None)
        elif self.signed.type == "targets":
        ...
        if role is None:
            raise exceptions.UnsignedMetadataError(
                f"No delegation found for {role_name}", delegate.signed
            )
        ...
        if len(signing_keys) < role.threshold:
            raise exceptions.UnsignedMetadataError(
                f"{role_name} was signed by {len(signing_keys)} keys out of "
                f"threshold of {role.threshold} keys",
                delegate.signed,
            )
...
class Key:
    ...
    def verify_signature(
        ...
        """Verifies that the metadata.signatures contains a signature signing
        metadata.signed, and that the signature was made with this key.
...
class Root(Signed):
    ...
    def add_key(self, role: str, key: Key) -> None:
        ...
        self.roles[role].keyids.add(key.id)
